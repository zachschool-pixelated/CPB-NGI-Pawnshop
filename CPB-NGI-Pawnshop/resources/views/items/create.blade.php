                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('items.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="safe_id" name="safe_id" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm">
                                ...
                                @foreach($safes as $safe)
                                    <option value="{{ $safe->id }}" @selected(old('safe_id') == $safe->id)>{{ $safe->safe_code }} - {{ $safe->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="appraised_value" class="block mt-1 w-full" type="number" name="appraised_value" :value="old('appraised_value')" step="0.01" required />
                        ...
                        <div>
                            ...
                            <select id="condition" name="condition" class="block mt-1 w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm" required>
                                ...
                                <option value="excellent" @selected(old('condition') === 'excellent')>{{ __('Excellent') }}</option>
                                <option value="good" @selected(old('condition') === 'good')>{{ __('Good') }}</option>
                                <option value="fair" @selected(old('condition') === 'fair')>{{ __('Fair') }}</option>
                                <option value="poor" @selected(old('condition') === 'poor')>{{ __('Poor') }}</option>
                            </select>
